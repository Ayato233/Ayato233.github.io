...
/** 分批渲染：每批渲染的图片数量。巨图相册（单张数 MB）大量一次性
 *  渲染会导致滚动时解码卡顿；分批 + 哨兵触底加载可显著缓解。 */
const PAGE_SIZE = 12;
		...
		observer = new IntersectionObserver(
			(entries) => {
				if (entries[0]?.isIntersecting) {
					...
					shownCount = Math.min(next, photos.length);
					if (shownCount >= photos.length) observer?.disconnect();
				}
			},
			{ rootMargin: "400px" },
		);
...
/** 重置展示数量（照片列表变化时）。 */
function resetShown(count: number) {
	shownCount = Math.min(PAGE_SIZE, count);
}
...
const batchedPhotos = $derived(visiblePhotos.slice(0, shownCount));
...
{#if batchedPhotos.length > 0}
			...
			>
		{#each batchedPhotos as photo, index (photo.id)}
			<button
				type="button"
	...
	{#if shownCount < photos.length}
		<div
			bind:this={sentinel}
			class="album-gallery__sentinel"
			aria-hidden="true"
		></div>
	{/if}
{:else}
