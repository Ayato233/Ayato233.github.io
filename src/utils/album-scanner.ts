...
/**
 * 若相册目录存在 thumbs/<basename>.webp 缩略图（由 generate-album-thumbs.mjs
 * 生成），则返回其 public 路径；否则 undefined（列表直接加载原图）。
 */
function localThumbnail(albumId: string, albumDir: string, fileName: string): string | undefined {
	const thumbName = `${path.basename(fileName, path.extname(fileName))}.webp`;
	const thumbPath = path.join(albumDir, THUMB_DIR, thumbName);
	if (fs.existsSync(thumbPath)) {
		return toPublicPath(`${albumId}/${THUMB_DIR}/${thumbName}`);
	}
	return undefined;
}
...
function localPhotos(albumId: string, albumDir: string): AlbumPhoto[] {
	return resolveLocalPhotoFiles(albumDir).map((fileName, index) => {
		...
		return {
			...
			thumbnail: localThumbnail(albumId, albumDir, fileName),
			alt: parsed.title,
			title: parsed.title,
			tags: parsed.tags,
